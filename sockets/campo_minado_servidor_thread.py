u""" CAMPO MINADO - Servidor com suporte a threads UDP """
...
u""" Implementação de um servidor com suporte a threads que faz uso de um socket UDP """
...
def server_thread():
    ...
    socketInstancia.bind(origem)

    while True:
        print("Servidor ativo..")
        data, address = socketInstancia.recvfrom(MAX_BYTES)
        # Criação de thread procedural
        t = threading.Thread(target=tratar_conexao, args=(socketInstancia, data, address))
        t.start()
